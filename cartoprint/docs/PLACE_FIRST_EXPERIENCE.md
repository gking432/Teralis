...
Illustrated Atlas uses a saved bitmap plus separately rendered personal caption. Tennessee and Wisconsin are the saved-art pilots. Madison uses the real-map Landmark Map described in CITY_LANDMARK_MAPS.md. Other states explicitly show that illustration is not available, with a link to Tennessee; no generic or mislabeled replacement is used. Adding a reviewed asset to `ILLUSTRATIONS` enables the edition for its state. Prepared art uses its registered portrait or landscape composition, original colors, and a shared preview/export geometry. This avoids runtime AI generation and preserves its design.
