 ...
 * Map Style Engine
 *
 * Handles greyscale conversion and style overrides for the liberty base style.
 * Ported from /reference/prototype.html — see applyGreyscale() and applyStyleOverrides().
 */
...
// Reusable 1x1 canvas for color parsing
...
/** Convert a CSS color string to greyscale */
...
/** Recursively convert color values in MapLibre expressions to greyscale */
...
/** Convert the entire map style to greyscale */
...
/** Apply style overrides for bolder borders, better zoom visibility, etc. */
export function applyStyleOverrides(map: MaplibreMap): void {
  ...
  style.layers.forEach((layer) => {
    ...
    // Hide natural_earth raster (colored terrain imagery)
    if (id === 'natural_earth') {
...
/** Add terrain hillshade source and layer */
export function addTerrain(map: MaplibreMap): void {
  ...
  // Insert hillshade below water layers
  map.addLayer(
    {
      id: 'hillshade-layer',
      type: 'hillshade',
    ...
    'waterway_tunnel' // Insert before this layer
  );
}
